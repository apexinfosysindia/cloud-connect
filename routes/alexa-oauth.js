...
/**
 * Alexa account-linking OAuth (the side WE host: Alexa is the OAuth client).
 * Mirror of routes/google-home-oauth.js.
 *
 *   GET  /api/alexa/oauth          authorize → consent → issue auth code
 *   POST /api/alexa/oauth/continue consent confirmation (sets portal cookie)
 *   POST /api/alexa/token          authorization_code + refresh_token grants
 */
module.exports = function ({ dbGet, dbRun, dbTransaction, config, utils, auth, core, eventGateway }) {
    ...
    router.get(
        ...
        asyncHandler(async (req, res) => {
            ...
            const authCode = core.generateAlexaOAuthCode();
            const nowIso = new Date().toISOString();
            const expiresAt = new Date(Date.now() + core.getAlexaAuthCodeTtlSeconds() * 1000).toISOString();

            await dbTransaction(async ({ dbRun: txRun }) => {
                await txRun(
                    `INSERT INTO alexa_auth_codes (user_id, code_hash, redirect_uri, scopes, expires_at, created_at)
                     VALUES (?, ?, ?, ?, ?, ?)`,
                    [user.id, utils.hashSecret(authCode), redirectUri, 'alexa::smart_home', expiresAt, nowIso]
                );
                await txRun(`UPDATE users SET alexa_linked = 1 WHERE id = ?`, [user.id]);
            });

            eventGateway.scheduleAlexaAddOrUpdateReportForUser(user.id, 'oauth_linked');
    ...
    router.post(
        ...
        asyncHandler(async (req, res) => {
            ...
            return res.status(200).json({ ok: true, redirect_url: authorizeUrl.toString() });
        })
    );
    ...
    router.post(
        ...
        asyncHandler(async (req, res) => {
            ...
            if (grantType === 'authorization_code') {
                ...
                const linkedUser = await core.findUserByAlexaAuthCode(code, redirectUri);
                if (!linkedUser) {
                ...
                await dbRun(`UPDATE alexa_auth_codes SET consumed_at = ? WHERE id = ?`, [
                    new Date().toISOString(),
                    linkedUser.oauth_code_id
                ]);
                const tokenResponse = await core.issueAlexaTokensForUser(linkedUser.id);
            ...
            if (grantType === 'refresh_token') {
                ...
                const refreshRow = await core.findAlexaRefreshTokenRow(refreshToken);
                if (!refreshRow) {
                    return res.status(400).json({ error: 'invalid_grant' });
                }
                ...
                const tokenResponse = await core.issueAlexaTokensForUser(user.id, refreshToken);
                return res.status(200).json(tokenResponse);
            }
