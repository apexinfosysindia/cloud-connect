...
// Amazon Alexa Smart Home skill OAuth (account linking) endpoints.
// Mirrors routes/google-home-oauth.js — same portal-session gated consent
// flow, same auth-code + refresh-token grants, just rebranded for Alexa
// and storing into alexa_auth_codes / alexa_tokens.

module.exports = function ({ dbGet, dbRun, dbTransaction, config, utils, auth, alexaCore, alexaEvents }) {
    ...
    router.get(
        ...
        asyncHandler(async (req, res) => {
            ...
            if (!alexaCore.isTrustedAlexaRedirectUri(redirectUri)) {
            ...
            await dbTransaction(async ({ dbRun: txRun }) => {
                await txRun(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                    `,
                    [user.id, utils.hashSecret(authCode), redirectUri, 'alexa_smart_home', expiresAt, nowIso]
                );
            ...
            if (alexaEvents) {
                try {
                    alexaEvents.scheduleAlexaDiscoveryUpdateForUser(user.id, 'oauth_linked');
                    alexaEvents.scheduleAlexaChangeReportForUser(user.id, { force: true });
                } catch (scheduleErr) {
                    console.error('ALEXA SCHEDULE ON LINK ERROR:', scheduleErr);
                }
            }
