...
const ENTITY_TYPE_TO_CATEGORY = {
    ...
    media_player: 'TV',
    scene: 'SCENE_TRIGGER',
    // script/automation expose PowerController (TurnOn/TurnOff), so they must use
    // a category compatible with PowerController. ACTIVITY_TRIGGER / SCENE_TRIGGER
    // require Alexa.SceneController and cause AddOrUpdateReport schema rejection.
    script: 'SWITCH',
    automation: 'SWITCH',
    ...
    switch (entityType) {
        ...
        case 'input_select': {
            ...
            for (const opt of rawOptions) {
                ...
                if (!friendly || seenFriendly.has(friendly)) continue;
                // Alexa mode value pattern: [A-Za-z0-9_\-.]
                let value = String(opt).replace(/[^A-Za-z0-9_\-.]/g, '_').slice(0, 128);
                if (!value) value = `opt_${supportedModes.length}`;
                // Ensure uniqueness of `value` within this ModeController (Alexa schema requires it).
                if (seenValues.has(value)) {
                    let suffix = 2;
                    let candidate = `${value.slice(0, 120)}_${suffix}`;
                    while (seenValues.has(candidate)) {
                        suffix += 1;
                        candidate = `${value.slice(0, 120)}_${suffix}`;
                    }
                    value = candidate;
                }
...
function buildAlexaEndpoint(entity, _userPin) {
    ...
    // Skip entity types Alexa cannot meaningfully expose. Returning an endpoint
    // whose only capability is Alexa.EndpointHealth (no controllable interface)
    // fails AddOrUpdateReport schema validation and rejects the entire payload.
    if (!Object.prototype.hasOwnProperty.call(ENTITY_TYPE_TO_CATEGORY, entityType)) {
        return null;
    }
