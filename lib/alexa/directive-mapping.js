...
function friendlyNamesText(...labels) {
    return {
        friendlyNames: labels.map((label) => ({
            '@type': 'text',
...
function resolveAlexaCapabilities(entityType, statePayload) {
    ...
    switch (entityType) {
        ...
        case 'select':
        case 'input_select': {
            const options = Array.isArray(statePayload.options)
            ...
            if (options.length > 0) {
                caps.push(
                    capabilityObject('Alexa.ModeController', ['mode'], {
                        ...
                        configuration: {
                            ordered: false,
                            supportedModes: options.map((opt) => ({
                                value: opt,
                                modeResources: friendlyNamesText(opt)
                            }))
                        }
                    })
                );
...
function buildAlexaEndpoint(entity, _userPin) {
    ...
    const manufacturer = safe(statePayload._manufacturer, 'Apex Infosys');
    const model = safe(statePayload._model, entityType);
    const softwareVersion = safe(statePayload._sw_version, '1.0.0');
    const friendlyName = safe(entity.display_name, entity.entity_id);

    return {
        endpointId: entity.entity_id,
        manufacturerName: manufacturer,
        description: `${friendlyName} (${entityType})`.slice(0, 128),
        friendlyName,
