...
function resolveAlexaCapabilities(entityType, statePayload) {
    ...
    switch (entityType) {
        ...
        case 'cover': {
            caps.push(capabilityObject('Alexa.RangeController', ['rangeValue'], { instance: 'Cover.Position' }));
            caps.push(capabilityObject('Alexa.EndpointHealth', ['connectivity']));
            return caps;
        }
        case 'valve': {
            caps.push(capabilityObject('Alexa.RangeController', ['rangeValue'], { instance: 'Valve.Position' }));
            caps.push(capabilityObject('Alexa.EndpointHealth', ['connectivity']));
            return caps;
        }
        case 'lock': {
        ...
        case 'climate': {
            caps.push(
                capabilityObject('Alexa.ThermostatController', [
                    'targetSetpoint',
                    'lowerSetpoint',
                    'upperSetpoint',
                    'thermostatMode'
        ...
        case 'water_heater': {
            caps.push(capabilityObject('Alexa.ThermostatController', ['targetSetpoint']));
            caps.push(capabilityObject('Alexa.TemperatureSensor', ['temperature']));
            caps.push(capabilityObject('Alexa.EndpointHealth', ['connectivity']));
            return caps;
        }
        ...
        case 'humidifier': {
            caps.push(capabilityObject('Alexa.PowerController', ['powerState']));
            caps.push(capabilityObject('Alexa.RangeController', ['rangeValue'], { instance: 'Humidifier.Humidity' }));
            caps.push(capabilityObject('Alexa.EndpointHealth', ['connectivity']));
            return caps;
        }
        ...
        case 'select':
        case 'input_select': {
            caps.push(capabilityObject('Alexa.ModeController', ['mode'], { instance: 'Select.Option' }));
            caps.push(capabilityObject('Alexa.EndpointHealth', ['connectivity']));
            return caps;
        }
        default: {
...
function buildAlexaEndpoint(entity, _userPin) {
    ...
    return {
        ...
        manufacturerName: statePayload._manufacturer || 'Apex Infosys',
        description: `${entity.display_name} (${entityType})`,
        friendlyName: entity.display_name || entity.entity_id,
        displayCategories: [category],
        additionalAttributes: {
            manufacturer: statePayload._manufacturer || 'Apex Infosys',
            model: statePayload._model || entityType,
            softwareVersion: statePayload._sw_version || '1.0.0',
            customIdentifier: String(entity.id || entity.entity_id)
        },
