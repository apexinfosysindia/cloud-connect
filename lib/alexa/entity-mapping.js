...
function assetText(text) {
    return { '@type': 'text', value: { text, locale: 'en-US' } };
}
...
function rangeControllerSpec({ instance, names, min, max, precision, unitOfMeasure, presets, semantics, readValue }) {
    const configuration = {
        supportedRange: { minimumValue: min, maximumValue: max, precision: precision || 1 }
    };
...
function securityPanelSpec(sp, userHasSecurityPin) {
    const discovery = {
        configuration: {
            ...
            // Disarm-only PIN: advertised only when the user configured one.
            supportedAuthorizationTypes: userHasSecurityPin ? [{ type: 'FOUR_DIGIT_PIN' }] : []
        }
    };
    return {
...
function mediaPlayerSpecs(sp) {
    ...
    // InputController — from source_list.
    ...
    if (has(MP_SELECT_SOURCE) && sources.length > 0) {
        specs.push({
            ...
            version: '3',
            properties: ['input'],
            discovery: { inputs: sources.map((s) => ({ name: String(s) })) },
...
function humidifierSpecs(sp) {
    ...
    specs.push(
        rangeControllerSpec({
            ...
            names: [assetId('Alexa.Setting.Humidity'), assetText('Humidity')],
            min,
            max,
            precision: 1,
...
function resolveDisplayCategories(entityType, statePayload) {
    ...
    if (entityType === 'sensor') {
        return [dc === SENSOR_HUMIDITY_CLASS ? 'HUMIDITY_SENSOR' : 'TEMPERATURE_SENSOR'];
    }
