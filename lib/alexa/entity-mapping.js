...
// DoorbellEventSource is proactive-only: no properties, no retrievable state.
// Amazon requires `proactivelyReported: true` DIRECTLY on the capability object
// (not inside a properties block) — emitted here via the discovery passthrough.
function doorbellSpec() {
    return {
        iface: 'Alexa.DoorbellEventSource',
        version: '3',
        discovery: { proactivelyReported: true },
        readState: () => []
    };
}

// CameraStreamController: advertises supported stream configs. HA produces HLS;
// H264/AAC, 720p. No retrievable state — the stream URI is fetched live on the
// InitializeCameraStreams directive. (Best-effort: some Echo Shows want RTSP.)
function cameraSpec() {
    return {
        iface: 'Alexa.CameraStreamController',
        version: '3',
        discovery: {
...
// HA hvac mode → Alexa thermostatMode. Amazon's enum is ONLY:
// AUTO, COOL, ECO, EM_HEAT, HEAT, OFF. HA modes with no equivalent (dry,
// fan_only) map to null and are DROPPED from supportedModes — emitting an
// invalid value like DEHUMIDIFY/FAN fails schema and 400s the whole batch.
const HA_TO_ALEXA_THERMOSTAT_MODE = {
    ...
    auto: 'AUTO',
    off: 'OFF'
    // fan_only, dry → null (no valid Alexa thermostatMode) → omitted.
};

function toAlexaThermostatMode(haMode) {
    if (!haMode) return 'OFF';
    return HA_TO_ALEXA_THERMOSTAT_MODE[String(haMode).toLowerCase()] || null;
}

/**
 * ThermostatController. `modes` = list of HA hvac/operation modes to advertise;
 * omit/empty for a setpoint-only device (water_heater). `single` advertises only
 * targetSetpoint; dual (single:false) also exposes lower/upperSetpoint — but ONLY
 * when the device actually reports a range (Amazon rejects lower/upperSetpoint on
 * a single-setpoint device).
 */
function thermostatSpec({ modes, single, dualCapable, readMode, readTarget, readLow, readHigh, unitKey }) {
    // Only advertise dual setpoints when the device is genuinely range-capable.
    const dual = !single && dualCapable !== false;
    const properties = ['targetSetpoint'];
    if (dual) {
        properties.push('lowerSetpoint', 'upperSetpoint');
    }
    // Map HA modes → valid Alexa enum, dropping unmappable ones.
    const alexaModes = Array.isArray(modes)
        ? Array.from(new Set(modes.map(toAlexaThermostatMode).filter(Boolean)))
        : [];
    if (alexaModes.length > 0) {
        properties.push('thermostatMode');
    }
    const configuration = {};
    if (alexaModes.length > 0) {
        configuration.supportedModes = alexaModes;
    }
    ...
    return {
        ...
        readState: (sp) => {
            ...
            if (alexaModes.length > 0 && readMode) {
                const m = toAlexaThermostatMode(readMode(sp));
                // Report a valid mode; fall back to OFF if current HA mode is unmappable.
                out.push({ name: 'thermostatMode', value: m || 'OFF' });
            }
            return out;
        }
    };
...
function buildCapabilityPlan(entityType, statePayload, options = {}) {
    ...
    if (isLight(entityType)) {
        ...
        // the automation; group is fanned out by the add-on via apexos.*)
        specs.push(powerSpec());
    } else if (
        entityType === 'scene' ||
        entityType === 'script' ||
        ...
        // Only doorbell events map to an Alexa interface; other event classes
        // have no equivalent and fall through to base+health → excluded.
        if (sp.device_class === 'doorbell') {
            specs.push(doorbellSpec());
        }
    }
...
function climateSpecs(sp) {
    ...
    // Dual setpoint only when HA advertises TARGET_TEMPERATURE_RANGE (bit 2) or a
    // heat_cool/auto mode. A cool-only AC is single-setpoint — advertising
    // lower/upperSetpoint on it fails Amazon's schema.
    const CLIMATE_TARGET_TEMP_RANGE = 2;
    const dualCapable = has(CLIMATE_TARGET_TEMP_RANGE) || hvacModes.some((m) => m === 'heat_cool' || m === 'auto');

    // Amazon's ThermostatController REQUIRES at least one of HEAT or COOL. A
    // climate entity with only dry/fan_only/off modes (e.g. a standalone
    // dehumidifier or a fan-only unit exposed as `climate.`) is NOT a valid
    // thermostat — exposing one would 400 the whole batch. Such entities are
    // surfaced via TemperatureSensor + their fan/preset/swing ModeControllers
    // only (no ThermostatController).
    const modesForThermostat = hvacModes.length > 0 ? hvacModes : ['off', 'heat', 'cool', 'auto'];
    const alexaThermoModes = Array.from(new Set(modesForThermostat.map(toAlexaThermostatMode).filter(Boolean)));
    const isRealThermostat = alexaThermoModes.includes('HEAT') || alexaThermoModes.includes('COOL');
    ...
    // TemperatureSensor (current room temperature). On a REAL thermostat the
    // Alexa app's dial widget needs a current temperature to render — without
    // one, the dial spins forever (target temp can't be seen or set), even
    // though the mode/fan controls work. So a thermostat ALWAYS advertises
    // TemperatureSensor and ALWAYS reports a value: the real ambient when the
    // device has a room sensor, otherwise the target setpoint as a stand-in
    // (better than a spinning, unusable card). Non-thermostat climates
    // (dehumidifier/fan-only) only get a sensor when they actually report temp.
    const climateHasAmbient = sp.ambient_temperature != null;
    if (isRealThermostat || climateHasAmbient) {
        specs.push({
            iface: 'Alexa.TemperatureSensor',
            version: '3',
            properties: ['temperature'],
            readState: (s) => {
                // Prefer the real room reading; fall back to the setpoint so the
                // dial always has a number to draw. Never emit nothing.
                let t = s.ambient_temperature;
                if (t == null) t = s.target_temperature;
                if (t == null) t = 22;
                return [{ name: 'temperature', value: tempValue(t, s.temperature_unit) }];
            }
        });
    }
...
function validateAlexaEndpoint(ep) {
    ...
    for (const c of caps) {
        ...
        if (c.interface === 'Alexa.ThermostatController') {
            const sm = (c.configuration || {}).supportedModes;
            const VALID_TMODES = new Set(['AUTO', 'COOL', 'ECO', 'EM_HEAT', 'HEAT', 'OFF']);
            for (const m of sm || []) if (!VALID_TMODES.has(m)) errs.push(`thermostatMode ${m}`);
            // Amazon requires at least one of HEAT or COOL on a ThermostatController.
            if (Array.isArray(sm) && !sm.includes('HEAT') && !sm.includes('COOL')) errs.push('thermostat: no HEAT/COOL mode');
            // lower/upperSetpoint may only appear together (dual-setpoint).
            const props = ((c.properties || {}).supported || []).map((p) => p.name);
            if (props.includes('lowerSetpoint') !== props.includes('upperSetpoint')) errs.push('thermostat: lone lower/upperSetpoint');
        }
    }
    if (!hasBase) errs.push('missing base Alexa interface');
    // Amazon requires CAMERA to be listed FIRST in displayCategories when an
    // endpoint has a CameraStreamController (and for video doorbells).
    const hasCamera = caps.some((c) => c.interface === 'Alexa.CameraStreamController');
    if (hasCamera && Array.isArray(ep.displayCategories) && ep.displayCategories[0] !== 'CAMERA') {
        errs.push('CAMERA must be first in displayCategories');
    }
    return errs;
}
 ...
 */
// Safe default value for a property Alexa expects but the device didn't report.
// Emitting SOMETHING for every advertised-retrievable property is mandatory:
// a missing property leaves ReportState incomplete and the device card spins
// forever (and can't be controlled). Defaults are inert/neutral.
function defaultPropertyValue(name) {
    switch (name) {
        case 'rangeValue':
            return 0;
        case 'mode':
            return null; // handled specially below (omit only if truly no modes)
        case 'toggleState':
            return 'OFF';
...
function parseAlexaEndpointState(endpoint) {
    ...
    for (const spec of plan) {
        ...
        const reportedNames = new Set(entries.map((e) => e.name));
        ...
        // Backfill any advertised RETRIEVABLE property the readState didn't emit,
        // so Alexa's ReportState response is always complete (no spinning card).
        // Only applies to specs that declare retrievable properties (retrievable
        // defaults to true when properties are present).
        if (Array.isArray(spec.properties) && spec.retrievable !== false) {
            for (const name of spec.properties) {
                if (reportedNames.has(name)) continue;
                let value = defaultPropertyValue(name);
                if (name === 'mode') {
                    // A ModeController must report one of its OWN supported modes.
                    const modes = spec.discovery && spec.discovery.configuration && spec.discovery.configuration.supportedModes;
                    if (Array.isArray(modes) && modes.length > 0) {
                        value = modes[0].value;
                    } else {
                        continue; // no modes to report → leave it out (shouldn't happen)
                    }
                }
                props.push({
                    namespace: spec.iface,
