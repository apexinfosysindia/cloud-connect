...
function climateSpecs(sp) {
    ...
    // TemperatureSensor (current room temperature) is only advertised when the
    // device actually reports an ambient temperature. Advertising it as
    // retrievable while reporting NO value makes Alexa's ReportState response
    // incomplete → the thermostat card spins forever and target temp can't be
    // set. Many bare AC units have no room sensor (ambient_temperature: null),
    // so this capability must be conditional.
    if (sp.ambient_temperature != null) {
        specs.push({
            iface: 'Alexa.TemperatureSensor',
            version: '3',
            properties: ['temperature'],
            // Always emit a value (the capability is only present when ambient
            // exists); fall back defensively so the property is never dropped.
            readState: (s) => {
                const amb = s.ambient_temperature != null ? s.ambient_temperature : s.target_temperature;
                return amb == null ? [] : [{ name: 'temperature', value: tempValue(amb, s.temperature_unit) }];
            }
        });
    }
