...
module.exports = function ({ dbGet, dbRun, dbAll, core }) {
    ...
    async function handleGatewayAuthError(userId, response) {
        ...
        if (status === 401 || code === 'INVALID_ACCESS_TOKEN_EXCEPTION') {
            try {
                // Force a refresh past the cache. Success ⇒ token was just stale.
                await getLwaAccessTokenForUser(userId, { forceRefresh: true });
                console.warn('ALEXA GATEWAY 401: cached token was stale, refreshed OK for user', userId);
                return false;
            } catch (error) {
                if (isAlexaRevocationError(error)) {
                    return await cleanupRevokedAlexaUser(userId, '401 then invalid_grant on refresh');
                }
                // Some other refresh failure — don't unlink on an ambiguous error.
                console.error('ALEXA GATEWAY 401 refresh error (not unlinking):', error?.message);
                return false;
            }
        }
