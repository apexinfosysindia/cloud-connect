...
module.exports = function ({ dbGet, dbRun, dbAll, core }) {
    ...
    // Amazon caps a Discovery event (AddOrUpdateReport / DeleteReport / Discover
    // response) — documented as 256 KB max payload and 300 endpoints/customer. A
    // large inventory (e.g. 134 endpoints, each carrying full capability defs) in a
    // SINGLE message blows past the size limit and Amazon 500s the whole batch — which
    // is why bulk hide/unlink failed while one-at-a-time (1 endpoint) worked. So we
    // split the endpoint list into safe chunks and post each as its own event.
    const ALEXA_MAX_ENDPOINTS_PER_MESSAGE = 50;
    const ALEXA_INTER_BATCH_DELAY_MS = 150;
    ...
    // POST a Discovery event in endpoint-chunks. buildEvent(chunk) returns the full
    // event envelope for one chunk. Returns a combined result: ok only if EVERY chunk
    // succeeded; on any failure it returns the first failing chunk's status/body so
    // callers (and the retry path) react correctly. Stops at the first hard failure.
    async function postEventChunked(accessToken, endpoints, buildEvent) {
        const batches = chunkArray(endpoints, ALEXA_MAX_ENDPOINTS_PER_MESSAGE);
        let lastOk = { ok: true, statusCode: 202, body: null };
        for (let i = 0; i < batches.length; i += 1) {
            const resp = await postEvent(accessToken, buildEvent(batches[i]));
            if (!resp.ok) {
                return resp; // surface the failure; retry path will re-send the whole set
            }
            lastOk = resp;
            if (i < batches.length - 1) {
                await new Promise((r) => setTimeout(r, ALEXA_INTER_BATCH_DELAY_MS));
            }
        }
        return lastOk;
    }
    ...
    async function sendAddOrUpdateReport(userId) {
        ...
        // Chunked: a large inventory in one AddOrUpdateReport exceeds Amazon's 256 KB
        // payload cap and 500s. Split into per-message batches.
        return await postEventChunked(accessToken, endpoints, (batch) => ({
            event: {
                header: {
    ...
    async function sendDeleteReport(userId, endpointIds) {
        ...
        // Chunked: a large delete (e.g. unlink/hide-all of 134 endpoints) in one
        // DeleteReport exceeds Amazon's payload cap and 500s — which left tiles
        // orphaned while one-at-a-time deletes (1 endpoint) succeeded. Split into
        // per-message batches; the endpoint list maps to {endpointId} objects.
        const endpointObjs = ids.map((endpointId) => ({ endpointId }));
        return await postEventChunked(accessToken, endpointObjs, (batch) => ({
            event: {
                header: {
                    namespace: 'Alexa.Discovery',
                    name: 'DeleteReport',
    ...
    // Backoff schedule (ms) for retrying a stuck unlink. Amazon's enablement state
    // is eventually-consistent after a rapid link/unlink, so SKILL_DISABLED on the
    // delete usually clears within a few minutes. We retry a handful of times then
    // give up (the user can always click Unlink again — #230).
    const ALEXA_UNLINK_RETRY_DELAYS_MS = [60_000, 180_000, 300_000];
