...
module.exports = function ({ dbGet, dbRun, dbAll, core }) {
    ...
    // recovers reasonably fast (first retry at 30s) and ramps up, with a long tail
    // to outlast a longer Amazon flap.
    // ~12.5 min total across 7 attempts, then give up (manual Unlink still works).
    const ALEXA_UNLINK_RETRY_DELAYS_MS = [30_000, 60_000, 60_000, 120_000, 120_000, 180_000, 180_000];
    ...
    // Front-loaded (first retry at 15s) and ramping, with a long tail to outlast a
    // longer flap. ~7.25 min total across 7 attempts, then give up (user can re-hide).
    const ALEXA_DELETE_RETRY_DELAYS_MS = [15_000, 30_000, 30_000, 60_000, 60_000, 120_000, 120_000];
