...
module.exports = function ({ dbGet, dbRun, dbAll, core }) {
    ...
    const ALEXA_RECENT_EVENTS_CAP = 200;
    function pushRecent(type, outcome, userId, statusCode, detail) {
        const buf = state.alexaEventGatewayRecent;
        if (!buf) return;
        buf.push({
        ...
        if (buf.length > ALEXA_RECENT_EVENTS_CAP) buf.splice(0, buf.length - ALEXA_RECENT_EVENTS_CAP);
    }
    ...
    function markSuccess(type, userId, statusCode) {
        ...
        pushRecent(type, 'sent', userId, statusCode, null);
    }
    ...
    function markFailure(type, userId, statusCode, reason) {
        ...
        m.last_user_id = String(userId);
        pushRecent(type, 'failed', userId, statusCode, reason || 'unknown_error');
        // Surface gateway failures on the Server Health app-event stream too.
        recordAppEvent('warn', 'alexa-gateway', `${type} failed`, { user_id: String(userId), status: statusCode || null, reason: reason || 'unknown_error' });
    }
    ...
    function markSkipped(type, userId, reason) {
        ...
        pushRecent(type, 'skipped', userId, null, reason || null);
    }
    ...
    // A connection-level error (vs. an HTTP error response). undici (Node fetch)
    // keeps TLS sockets in a keep-alive pool; when the far end silently drops an
    // idle socket, the NEXT request grabs that dead socket and the first read fails
    // with ECONNRESET (surfaced as `TypeError: fetch failed` with a `cause`). This
    // is not an Amazon rejection — the request never landed — so re-dialing on a
    // fresh connection succeeds. Detect that class so we can retry it transparently.
    function isTransientConnectionError(error) {
        ...
        // undici wraps socket drops as a bare "fetch failed" TypeError.
        return error?.name === 'TypeError' && /fetch failed/i.test(error?.message || '');
    }

    async function postEventOnce(accessToken, event) {
    ...
    async function postEvent(accessToken, event) {
        try {
            return await postEventOnce(accessToken, event);
        } catch (error) {
            // Only a stale-socket/connection drop is safely retryable here: the POST
            // never reached Amazon, so re-sending can't double-apply. A brief pause
            // lets the dead socket clear so the retry opens a fresh connection. Any
            // other error (or a second failure) propagates to the caller as before.
            if (!isTransientConnectionError(error)) {
                throw error;
            }
            console.warn('ALEXA GATEWAY: transient connection error, re-dialing once —', error?.cause?.code || error?.message);
    ...
    // clears on its own within seconds-to-minutes. Monotonic ramp from 30s with a
    // long tail to outlast a longer Amazon flap (same shape as the hide/expose
    // retry). ~9.5 min total across 7 attempts, then give up (manual Unlink still works).
    const ALEXA_UNLINK_RETRY_DELAYS_MS = [30_000, 60_000, 60_000, 90_000, 90_000, 120_000, 120_000];
    ...
    // Monotonic ramp from 30s with a long tail to outlast a longer flap.
    // ~9.5 min total across 7 attempts, then give up (user can re-hide).
    const ALEXA_DELETE_RETRY_DELAYS_MS = [30_000, 60_000, 60_000, 90_000, 90_000, 120_000, 120_000];
