const state = {
    ...
    // Per-user in-flight unlink promise, keyed by userId → Promise<{tiles_cleared}>.
    // Single-flight lock for the gated unlink: two simultaneous Unlink clicks JOIN
    // the same operation instead of racing. Without it, one click's cleanup
    // (token wipe) can land under the other click's in-flight DeleteReport,
    // orphaning tiles. Cleared when the unlink settles.
    alexaUnlinkInflight: new Map(),
    ...
    // Ring buffer of the most recent proactive-gateway events (newest last), for the
    // admin "recent events" log stream. In-memory only — bounded and volatile (clears
    // on restart, like the counters above). Each entry:
    //   { at, type, outcome: 'sent'|'failed'|'skipped', user_id, status, detail }
    alexaEventGatewayRecent: []
};
