        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                if (!response?.ok && !response?.skipped) {
                    ...
                    // Capture one sample per DISTINCT entity_type (no cap) + per-type count,
                    // so the offending entity type shows up in the log even when >8 types exist.
                    const byType = new Map();
                    const countByType = new Map();
                    for (const ep of endpoints) {
                        const t = ep?.cookie?.entity_type || 'unknown';
                        countByType.set(t, (countByType.get(t) || 0) + 1);
                        if (!byType.has(t)) byType.set(t, ep);
                    }
                    console.warn('ALEXA DISCOVERY UPDATE FAILED:', {
                        ...
                        type_counts: Object.fromEntries(countByType.entries()),
                        samples: Object.fromEntries(
                            [...byType.entries()].map(([t, ep]) => [t, JSON.stringify(ep).slice(0, 4000)])
                        )
                    });
                    return;
                }
