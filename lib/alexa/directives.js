...
// Snap a thermostat setpoint to the device's resolution. Alexa works in 0.5C
// steps; many ACs only accept whole degrees, so without snapping a "23" press
// arrives as 22.5, HA rounds it to 22, and Alexa's model drifts by half a degree
// indefinitely. We snap to `target_temp_step` when the device reports one, else
// to whole degrees (half-up, so 22.5 -> 23 matches an upward press). Reporting
// the snapped value back in optimisticState re-aligns Alexa so it then sends
// whole degrees.
function snapSetpoint(value, state) {
    const v = Number(value);
    if (!Number.isFinite(v)) return value;
    const step = Number(state && state.target_temp_step) > 0 ? Number(state.target_temp_step) : 1;
    ...
    if (name === 'SetTargetTemperature' || name === 'AdjustTargetTemperature') {
        ...
        if (!isWaterHeater && lower != null && upper != null) {
            return {
                ...
                payload: { target_temp_low: snapSetpoint(lower, state), target_temp_high: snapSetpoint(upper, state) },
                optimisticState: { target_temp_low: snapSetpoint(lower, state), target_temp_high: snapSetpoint(upper, state) }
            };
        }
        let temperature;
        ...
        // Snap to the device's resolution. Alexa works in 0.5C steps; many ACs
        // only accept whole degrees, so Alexa sends e.g. 22.5 when you press 23,
        // HA rounds it to 22, but Alexa never learns the device's real value and
        // keeps drifting by .5. Snapping here AND reporting the snapped value in
        // optimisticState re-aligns Alexa's model so it sends whole degrees next.
        temperature = snapSetpoint(temperature, state);
        if (isWaterHeater) {
