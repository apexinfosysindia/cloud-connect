...
function generateAdminConnectToken() {
    return config.ADMIN_CONNECT_TOKEN_PREFIX + crypto.randomBytes(24).toString('hex');
}
...
function getAdminConnectTokenTtlMinutes() {
    if (!Number.isFinite(config.ADMIN_CONNECT_TOKEN_TTL_MINUTES)) {
        return 10;
    }

    return Math.max(3, Math.min(60, Math.round(config.ADMIN_CONNECT_TOKEN_TTL_MINUTES)));
}
...
module.exports = {
    ...
    getAdminConnectTokenTtlMinutes,
