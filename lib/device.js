module.exports = function ({ dbGet, dbRun, dbAll, dbTransaction, config, utils }) {
    ...
    async function insertDeviceLog(deviceId, level, eventType, message, payload = null) {
        ...
        // Stamp the row explicitly (ISO 8601 UTC) so the denormalized
        // devices.last_event_at exactly matches the device_logs.created_at we
        // just wrote, instead of relying on two separate CURRENT_TIMESTAMP reads.
        const nowIso = new Date().toISOString();

        // One transaction: append the log, prune to the 250 newest, then refresh
        // the denormalized summary on the devices row (log_count + latest event).
        // The fleet list reads those columns directly, avoiding per-row subqueries.
        await dbTransaction(async ({ dbGet: txGet, dbRun: txRun }) => {
            await txRun(
                `
                INSERT INTO device_logs (device_id, level, event_type, message, payload, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
            `,
                [deviceId, normalizedLevel, normalizedEventType, normalizedMessage, payloadText, nowIso]
            );
            ...
            // Recompute log_count AFTER the prune (it is capped at 250, so an
            // increment would overshoot). The newest surviving row is the event
            // we just inserted unless the prune removed it, so re-read it.
            const summary = await txGet(
                `
                SELECT
                    (SELECT COUNT(*) FROM device_logs WHERE device_id = ?) AS log_count,
                    level AS last_event_level,
                    event_type AS last_event_type,
                    message AS last_event_message,
            ...
            `,
                [
                    ...
                    summary ? summary.last_event_level : null,
                    summary ? summary.last_event_type : null,
                    summary ? summary.last_event_message : null,
                    summary ? summary.last_event_at : null,
                    deviceId
                ]
            );
        });
