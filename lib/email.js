...
module.exports = function ({ dbGet, dbRun, config, utils }) {
    ...
    async function createEmailVerificationToken(userId) {
        ...
        await dbRun(`INSERT INTO email_verification_tokens (user_id, token_hash, expires_at) VALUES (?, ?, ?)`, [
            userId,
            tokenHash,
            expiresAt
        ]);
    ...
    async function markEmailVerificationTokenUsed(tokenId) {
        await dbRun(`UPDATE email_verification_tokens SET used_at = CURRENT_TIMESTAMP WHERE id = ?`, [tokenId]);
    }
    ...
    async function createPasswordResetToken(userId) {
        ...
        await dbRun(`INSERT INTO password_reset_tokens (user_id, token_hash, expires_at) VALUES (?, ?, ?)`, [
            userId,
            tokenHash,
            expiresAt
        ]);
    ...
    async function markPasswordResetTokenUsed(tokenId) {
        await dbRun(`UPDATE password_reset_tokens SET used_at = CURRENT_TIMESTAMP WHERE id = ?`, [tokenId]);
    }

    // ── Login OTP (passwordless-login emergency fallback) ───────────────────
    // A 6-digit numeric code, emailed when a passkey-holding customer falls back
    // to password sign-in. We persist only sha256(code); the raw code lives only
    // in the email. Mirrors the *_tokens helpers above but adds a per-code
    // attempt counter so a single issued code can't be brute-forced within the
    // per-IP rate limit (a 6-digit space is only 1,000,000 wide).
    async function createLoginOtp(userId) {
        // Invalidate any existing unused codes for this user — only the newest
        // code should ever be live.
        await dbRun(`UPDATE login_otp_tokens SET used_at = CURRENT_TIMESTAMP WHERE user_id = ? AND used_at IS NULL`, [
            userId
        ]);

        // crypto.randomInt is uniform over [0, 1000000) — zero-pad to a fixed
        // 6 digits so "001234" is a valid, distinguishable code.
        const code = String(crypto.randomInt(0, 1000000)).padStart(6, '0');
        const codeHash = utils.hashSecret(code);
        const expiresAt = new Date(Date.now() + config.LOGIN_OTP_TTL_MS).toISOString();

        await dbRun(`INSERT INTO login_otp_tokens (user_id, token_hash, expires_at) VALUES (?, ?, ?)`, [
            userId,
            codeHash,
            expiresAt
        ]);
    ...
    // Verify a submitted code for a user. Returns:
    //   { ok: true, id }          — matched a live code (caller marks it used)
    //   { locked: true }          — the live code is locked after too many tries
    //   { ok: false }             — no match (wrong/expired code)
    // A miss bumps `attempts` on the newest live code; once it reaches the cap
    // the code is dead even if the next guess is correct, so the user must
    // request a fresh one.
    async function verifyLoginOtp(userId, code) {
        // The current live (unused, unexpired) code for this user, if any.
        const live = await dbGet(
            `SELECT id, token_hash, attempts FROM login_otp_tokens
        ...
        if (Number(live.attempts) >= config.LOGIN_OTP_MAX_ATTEMPTS) {
            return { locked: true };
        }

        const codeHash = utils.hashSecret(String(code || ''));
        if (codeHash === live.token_hash) {
            return { ok: true, id: live.id };
        }

        // Wrong guess: burn an attempt against this exact code.
        await dbRun(`UPDATE login_otp_tokens SET attempts = attempts + 1 WHERE id = ?`, [live.id]);
        // If that tipped it over the cap, report locked so the UI can tell the
        // user to request a new code rather than keep guessing.
        if (Number(live.attempts) + 1 >= config.LOGIN_OTP_MAX_ATTEMPTS) {
            return { locked: true };
        }
        return { ok: false };
    }

    async function markLoginOtpUsed(tokenId) {
        await dbRun(`UPDATE login_otp_tokens SET used_at = CURRENT_TIMESTAMP WHERE id = ?`, [tokenId]);
    }
    ...
    // ── Login OTP email (transactional, NOT best-effort) ────────────────────
    // Unlike the security-notification senders below, the sign-in code is
    // something the user is actively waiting on: it sends through the direct
    // Resend path (not notifySecurityEvent) so the caller can gate on
    // isEmailConfigured() and surface a real error if delivery fails, instead of
    // silently no-op'ing and leaving the user stuck on the OTP screen.
    async function sendLoginOtpEmail(userEmail, code) {
        const minutes = Math.max(1, Math.round(config.LOGIN_OTP_TTL_MS / (60 * 1000)));
        const safeCode = escapeHtml(code);

        await getResendClient().emails.send({
            from: `${config.RESEND_FROM_NAME} <${config.RESEND_FROM_EMAIL}>`,
            to: [userEmail],
            subject: `${code} is your Oasis sign-in code`,
            text: [
                'Your Oasis Cloud sign-in code is:',
                '',
                `    ${code}`,
                '',
                `Enter it on the sign-in page to finish logging in. This code expires in ${minutes} minutes.`,
                '',
                "If you didn't try to sign in, you can ignore this email — your account stays secure. Never share this code with anyone.",
                '',
            ...
            html: buildNotificationEmailHtml({
                ...
                body:
                    'Use this code to finish signing in to your Oasis Cloud account:' +
                    '<div style="margin: 24px 0; padding: 18px 0; text-align: center; background: #f1f5f9; border-radius: 10px;">' +
                    `<span style="font-family: 'SFMono-Regular', Consolas, 'Liberation Mono', Menlo, monospace; font-size: 34px; font-weight: 700; letter-spacing: 10px; color: #0f172a;">${safeCode}</span>` +
                    '</div>' +
                    `This code expires in ${minutes} minutes.`,
                footnote:
                    "If you didn't try to sign in, you can ignore this email &mdash; your account stays secure. Never share this code with anyone."
            })
        });
    }
    ...
    // Welcome email for brand-new accounts created via SSO (Google/Microsoft/
    // Apple). Those signups skip email verification (the IdP already verified the
    // address), so this replaces the "verify your email" mail with a friendly
    // confirmation + a pointer to finish setup (cloud address + plan). Best-effort
    // via notifySecurityEvent, like the other notification emails — never blocks.
    function sendWelcomeEmail(userEmail) {
        const loginUrl = `${getPortalBaseUrl()}/login`;
        return notifySecurityEvent({
            to: userEmail,
            subject: 'Welcome to Oasis Cloud',
            text: [
                'Your Oasis Cloud account has been created using single sign-on.',
                '',
                'To finish setting up, sign in and choose your cloud address, then pick a plan to activate remote access.',
                '',
                loginUrl,
                '',
                'Oasis Cloud - Secure remote access for your smart home',
            ...
            html: buildNotificationEmailHtml({
                heading: 'Welcome to Oasis Cloud',
                body: 'Your account has been created using single sign-on. To finish setting up, sign in and choose your cloud address, then pick a plan to activate remote access.',
                ctaUrl: loginUrl,
                ctaLabel: 'Go to your account',
                footnote: "If you didn't create this account, please contact support."
            })
        });
    }
