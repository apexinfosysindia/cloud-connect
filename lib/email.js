...
module.exports = function ({ dbGet, dbRun, config, utils }) {
    ...
    async function createLoginOtp(userId) {
        ...
        await dbRun(
            `UPDATE login_otp_tokens SET used_at = CURRENT_TIMESTAMP WHERE user_id = ? AND used_at IS NULL`,
            [userId]
        );
        ...
        await dbRun(
            `INSERT INTO login_otp_tokens (user_id, token_hash, expires_at) VALUES (?, ?, ?)`,
    ...
    async function markLoginOtpUsed(tokenId) {
        await dbRun(
            `UPDATE login_otp_tokens SET used_at = CURRENT_TIMESTAMP WHERE id = ?`,
            [tokenId]
        );
    }
