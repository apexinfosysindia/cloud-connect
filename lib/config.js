...
// ── Alexa Smart Home (v2 rebuild) ──────────────────────────────────────────
// TTLs default to the same values the Google side uses — they're not Amazon
// requirements, just sensible bounds for OAuth code / access token / queued
// command lifetimes. lib/alexa/core.js clamps these to safe ranges.
...
// How long the smart-home directive handler will wait for the addon to
// report a result before giving up and returning ENDPOINT_UNREACHABLE.
// Must be < Alexa's ~8s skill response budget; default 5000ms leaves a
// comfortable buffer for the round-trip back to the customer.
const ALEXA_CONTROL_RESPONSE_TIMEOUT_MS = Number(
    process.env.ALEXA_CONTROL_RESPONSE_TIMEOUT_MS || 5000
);
// Allowed Alexa-side redirect_uri hosts. Amazon publishes three (one per
// region); we accept all three by default since a customer might link from
// any of them depending on their Amazon account locale.
const ALEXA_REDIRECT_URI_HOSTS = (
    process.env.ALEXA_REDIRECT_URI_HOSTS ||
    'pitangui.amazon.com,layla.amazon.com,alexa.amazon.co.jp'
)
    .split(',')
    .map((item) => item.trim().toLowerCase())
    .filter(Boolean);
...
module.exports = {
    ...
    ALEXA_CONTROL_RESPONSE_TIMEOUT_MS,
