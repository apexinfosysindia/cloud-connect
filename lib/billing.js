...
module.exports = function ({ dbGet, dbRun, dbAll, dbTransaction, config, createUniqueAccessToken }) {
    ...
    async function cancelSubscription(userId, options = {}) {
        ...
        // Inspect Razorpay's view of the subscription before deciding what to do.
        // Razorpay refuses the cancel_at_cycle_end PARAMETER (in either form)
        // for a sub in 'authenticated' state with no active billing cycle
        // (paid_count=0, start_at in the future) — both true and false are
        // rejected with "Subscription cannot be cancelled since no billing
        // cycle is going on". A parameterless cancel call works in this state
        // and properly terminates the mandate on Razorpay's side, so future
        // charges become impossible. cancel_at_cycle_end is meaningless here
        // anyway because there is no current cycle to wait out.
        ...
        if (preCycleSoftCancel) {
            // Pre-cycle subs (authenticated, paid_count=0, future start_at)
            // refuse the cancel_at_cycle_end parameter in EITHER value, but a
            // parameterless cancel call works and properly terminates the
            // mandate on Razorpay's side. cancel_at_cycle_end is meaningless
            // here anyway because there is no current cycle to wait out.
            ...
            try {
                ...
                console.error(
                    `Razorpay cancel failed for ${user.email} sub=${user.razorpay_subscription_id} atCycleEnd=${atCycleEnd}: "${rawMsg}"`
                );
                // Two-step fallback: try the opposite cancel_at_cycle_end mode,
                // then fall back to a parameterless cancel which Razorpay
                // accepts in states where the parameter itself is rejected.
                try {
                    cancelledSub = await razorpay.subscriptions.cancel(
                        user.razorpay_subscription_id,
                        { cancel_at_cycle_end: !atCycleEnd }
                    ...
                    try {
                        ...
                        const finalMsg = getBillingErrorMessage(
                            finalErr,
                            'Unable to cancel subscription'
                        );
                        console.error(
                            `Razorpay parameterless cancel also failed for ${user.email}: "${finalMsg}"`
                        );
                        const wrapped = new Error(finalMsg);
                        wrapped.cause = finalErr;
                        throw wrapped;
                    }
                }
            }
        ...
        // Trial users who cancel lose access immediately — otherwise the abuse
        // pattern is: sign up, get approved for trial, authenticate a card via
        // the annual subscription flow, cancel before start_at to dodge any
        // future charge, and keep using the service free for the whole trial
        // window. Active/paid users keep access until period end (handled by
        // the webhook flipping them to 'expired' when Razorpay closes the sub).
        const trialAbort = user.status === 'trial';
        try {
            if (trialAbort) {
                // Also record the trial as consumed so they cannot
                // delete-and-resignup for another free year on the same email.
                try {
                ...
                await dbRun(
                    `UPDATE users
                     SET razorpay_subscription_status = ?,
        ...
        console.log(
            `Subscription cancelled: user=${user.email} sub=${user.razorpay_subscription_id} at_cycle_end=${effectiveAtCycleEnd} status=${statusFromRzp} trial_abort=${trialAbort}`
        );
        return {
            cancelled: true,
