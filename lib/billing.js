...
module.exports = function ({ dbGet, dbRun, dbAll, dbTransaction, config, createUniqueAccessToken }) {
    ...
    async function cancelSubscription(userId, options = {}) {
        ...
        // Decide whether to cancel immediately or at cycle end, based on the
        // Razorpay subscription's real state. Razorpay rejects
        // cancel_at_cycle_end=true for subscriptions that haven't entered an
        // active billing cycle yet (status='authenticated' with start_at in
        // the future — the typical annual-trial setup). For those we must use
        // immediate cancel, which is safe because no money has been charged.
        let preferImmediate = false;
        try {
            const sub = await razorpay.subscriptions.fetch(user.razorpay_subscription_id);
            const nowEpoch = Math.floor(Date.now() / 1000);
            const startInFuture = sub.start_at != null && sub.start_at > nowEpoch;
            const noActiveCycleStatus = ['created', 'authenticated', 'pending'].includes(
                String(sub.status || '').toLowerCase()
            );
            if (startInFuture || noActiveCycleStatus) {
                preferImmediate = true;
            }
        } catch (fetchErr) {
            // If the fetch fails we fall back to the caller's preference and
            // rely on the error-path retry below.
            console.error(
        ...
        const firstAttemptAtCycleEnd = atCycleEnd && !preferImmediate;
        let cancelledSub;
        let effectiveAtCycleEnd = firstAttemptAtCycleEnd;
        try {
            cancelledSub = await razorpay.subscriptions.cancel(
                user.razorpay_subscription_id,
                { cancel_at_cycle_end: firstAttemptAtCycleEnd }
            );
        } catch (err) {
            // Fallback: if we tried cancel_at_cycle_end=true and Razorpay
            // refused, retry with immediate. No money has been charged in
            // the "no active cycle" case, so this is safe.
            const rawMsg = getBillingErrorMessage(err, '') || '';
            console.error(
                `Razorpay cancel failed for ${user.email} sub=${user.razorpay_subscription_id} atCycleEnd=${firstAttemptAtCycleEnd}: "${rawMsg}"`
            );
            if (firstAttemptAtCycleEnd) {
                try {
                    cancelledSub = await razorpay.subscriptions.cancel(
                        user.razorpay_subscription_id,
                        { cancel_at_cycle_end: false }
                    ...
                    console.error(
                        `Razorpay immediate-cancel fallback also failed for ${user.email}: "${message}"`
                    );
                ...
                const message = getBillingErrorMessage(err, 'Unable to cancel subscription');
                const wrapped = new Error(message);
                wrapped.cause = err;
                throw wrapped;
            }
