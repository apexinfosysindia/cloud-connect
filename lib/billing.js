...
module.exports = function ({ dbGet, dbRun, dbAll, dbTransaction, config, createUniqueAccessToken }) {
    ...
    async function cancelSubscription(userId, options = {}) {
        ...
        try {
            ...
            // Razorpay rejects cancel_at_cycle_end=true for subscriptions where
            // no billing cycle has started yet (e.g. annual trial subscription
            // in 'authenticated' state with start_at in the future). For those
            // we fall back to immediate cancel — safe because no money has
            // been charged, so there is no "remaining paid period" to preserve.
            const rawMsg = (err && (err.error?.description || err.message)) || '';
            const noCycleYet =
                atCycleEnd && /no billing cycle|cycle is going on|cycle_end/i.test(rawMsg);
            if (noCycleYet) {
                try {
                    cancelledSub = await razorpay.subscriptions.cancel(
                        user.razorpay_subscription_id,
