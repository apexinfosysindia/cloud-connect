...
module.exports = function ({ dbGet, dbRun, dbAll, dbTransaction, config, createUniqueAccessToken }) {
    ...
    async function cancelSubscription(userId, options = {}) {
        ...
        console.log(
            `Subscription cancelled: user=${user.email} sub=${user.razorpay_subscription_id} at_cycle_end=${atCycleEnd} status=${statusFromRzp}`
        );
        return { cancelled: true, atCycleEnd, subscription: cancelledSub, user };
    }
