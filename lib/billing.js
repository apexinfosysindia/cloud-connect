...
module.exports = function ({ dbGet, dbRun, dbAll, config, createUniqueAccessToken }) {
    ...
    async function getOrCreateCustomer(user) {
        ...
        // "Customer already exists for the merchant". This handles the case
        // where a customer record exists on Razorpay's side but our local
        // razorpay_customer_id column is empty (e.g. earlier checkout
        // attempt that didn't persist, or DB wiped but Razorpay retained).
    ...
    async function activateUserAccount(subscriptionId, paymentId, subscriptionStatus = 'active') {
        ...
        if (isTrial) {
            ...
            await recordTrialConsumed(user, 'subscription', { paymentFingerprint });
        } else {
    ...
    async function updateUserStatus(userId, status, options = {}) {
        ...
        if (status === 'trial') {
            await recordTrialConsumed(user, 'admin');
        }
