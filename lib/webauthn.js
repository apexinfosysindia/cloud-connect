...
module.exports = function ({ dbGet, dbRun, dbAll, config, utils }) {
    ...
    function serializeCredential(row) {
        ...
        // "iCloud Keychain"). This is the most useful label for telling passkeys
        // apart — especially duplicates enrolled across different managers, which
        // the browser-derived nickname ("Chrome on macOS") cannot distinguish.
        // Falls back to the stored UA nickname when the AAGUID is zero/unknown
        // (notably hybrid/cross-device credentials, which carry an all-zero
        // AAGUID for privacy). The raw nickname is still returned so the client
        // can show the device/method as secondary context.
