...
module.exports = function ({ dbGet, dbRun, dbAll, config, utils }) {
    ...
    function getRpConfig(kind) {
        if (kind === 'admin') {
            const rpID = envValue('WEBAUTHN_RP_ID_ADMIN') || config.ADMIN_PORTAL_HOST;
            const origin = envValue('WEBAUTHN_ORIGIN_ADMIN') || `https://${config.ADMIN_PORTAL_HOST}`;
            return { rpID, origin, rpName: 'ApexOS Cloud Connect Admin' };
        }
        const rpID = envValue('WEBAUTHN_RP_ID_CUSTOMER') || config.CUSTOMER_PORTAL_HOST;
        const origin = envValue('WEBAUTHN_ORIGIN_CUSTOMER') || `https://${config.CUSTOMER_PORTAL_HOST}`;
        return { rpID, origin, rpName: 'ApexOS Cloud Connect' };
    }
    ...
    // WebAuthn caps user.id at 64 bytes. The customer id is a small integer;
    // the admin email can be long, so derive a short stable handle from it.
    function deriveUserHandle(principal) {
        ...
        return crypto
            .createHash('sha256')
            .update(String(principal.subject || ''))
            .digest('hex')
            .slice(0, 32);
    }
    ...
    async function beginRegistration(principal) {
        ...
        const options = await generateRegistrationOptions({
            ...
            authenticatorSelection: {
                // Discoverable credentials + required user verification. Apple/
                // iCloud Keychain enroll under 'preferred', but third-party
                // managers (Dashlane, 1Password, Microsoft Authenticator) only
                // reliably offer to save a passkey when these are 'required'.
