...
module.exports = function ({ dbGet, dbRun, dbAll, config, utils }) {
    ...
    async function beginRegistration(principal) {
        ...
        const options = await generateRegistrationOptions({
            ...
            // ES256 (-7) + RS256 (-257) only. SimpleWebAuthn's default also
            // includes Ed25519 (-8); major sites (and what works reliably with
            // third-party authenticators) send just these two, so we drop -8 to
            // match and avoid authenticators that mishandle the extra algorithm.
