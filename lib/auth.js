...
module.exports = function ({ dbGet, config, utils, device, googleCore, alexaCore }) {
    ...
    // Pick whichever of the cookie/query tokens actually VERIFIES, preferring the
    // cookie only when it is valid. The OAuth authorize routes used to do
    // `cookieToken || queryToken`, so a STALE/invalid cookie would shadow a fresh
    // valid query token — the request then failed verification and bounced back to
    // /login, producing an infinite account-linking loop (esp. for Alexa, whose
    // client navigated straight to authorize without a cookie-refresh step). By
    // returning the first token that verifies, a bad cookie can no longer mask a
    // good query token. Returns { token, session } or { token: '', session: null }.
    function pickValidPortalToken(cookieToken, queryToken) {
        const cookie = typeof cookieToken === 'string' ? cookieToken.trim() : '';
        const query = typeof queryToken === 'string' ? queryToken.trim() : '';
        const cookieSession = cookie ? verifyPortalSessionToken(cookie) : null;
        if (cookieSession) {
            return { token: cookie, session: cookieSession };
        }
        const querySession = query ? verifyPortalSessionToken(query) : null;
        if (querySession) {
            return { token: query, session: querySession };
        }
