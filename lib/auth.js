...
module.exports = function ({ dbGet, config, utils, device, googleCore, alexaCore }) {
    ...
    function requireAlexaForwarderSecret(req, res, next) {
        const configured = config.ALEXA_FORWARDER_SHARED_SECRET || '';
        if (!configured) {
            return res.status(503).json({ error: 'forwarder_secret_not_configured' });
        }
        const provided = req.header('X-Alexa-Forwarder-Secret') || '';
        if (provided !== configured) {
            return res.status(401).json({ error: 'invalid_forwarder_secret' });
        }
        return next();
    }
