...
module.exports = function ({ dbGet, dbRun, dbAll, config, utils, state, entityMapping }) {
    ...
    async function sendGoogleReportState(agentUserId, statesByEntityId, requestId = null) {
        ...
        const response = await postToGoogleHomegraph('/v1/devices:reportStateAndNotification', buildPayload());

        // A 404 here is Google saying it doesn't know this user/these devices YET —
        // it happens when a ReportState races ahead of the initial RequestSync (right
        // after linking or a server restart). Google's own error text says to send a
        // RequestSync to register the inventory. So: fire a SYNCHRONOUS RequestSync
        // (async:false delivers the SYNC to our fulfillment before it returns, so the
        // devices are registered), then retry the report ONCE. This makes the first
        // report land instead of surfacing a 404 the user shouldn't see.
        if (response?.statusCode === 404) {
            console.warn('GOOGLE REPORT STATE 404 (devices not registered) — sending RequestSync then retrying once for', normalized);
            try {
                await sendGoogleRequestSync(normalized, { sync: true });
            } catch (error) {
                console.warn('GOOGLE REPORT STATE 404 recovery: RequestSync failed —', error?.message);
                return response; // surface the original 404; nothing more we can do
            }
            return await postToGoogleHomegraph('/v1/devices:reportStateAndNotification', buildPayload());
        }
