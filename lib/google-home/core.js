...
module.exports = function ({ dbGet, dbRun, dbAll, homegraph }) {
    ...
    async function upsertGoogleEntityFromDevice(userId, deviceId, payload) {
        ...
        // HA device-registry identity, used by the portal to group entities by
        // their Home Assistant device. Stored alongside the other _-prefixed
        // device-registry hints rather than in a dedicated column.
        if (payload?.ha_device_id) stateObj._ha_device_id = String(payload.ha_device_id).slice(0, 120);
        if (payload?.ha_device_name) stateObj._ha_device_name = String(payload.ha_device_name).slice(0, 120);
        ...
        if (existing) {
            ...
            // Domains that are rarely meant for voice exposure are inserted
            // hidden by default; the user can still expose them manually from
            // the Oasis portal. (automation/script: side-effecty toggles;
            // button/input_select/select: stateless or multi-option helpers.)
            const DEFAULT_HIDDEN_DOMAINS = new Set([
                'automation',
                'script',
                'button',
                'input_select',
            ...
            const defaultExposed = DEFAULT_HIDDEN_DOMAINS.has(entityType) ? 0 : 1;
