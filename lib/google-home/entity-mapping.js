...
function buildGoogleDeviceObject(entity, userSecurityPin) {
    ...
    const deviceObj = {
        ...
        deviceInfo: {
            // ALWAYS report Apex Infosys as the manufacturer — never the
            // downstream device's brand. The Apex MCU is a hub/bridge: the
            // lights/switches behind it are third-party (often Tuya-based), and
            // Home Assistant's registry reports THOSE brands. Forwarding them
            // made Google's certification tooling see the submission's
            // manufacturer flip from "Apex Infosys" to "Tuya" between reviews,
            // triggering a "clarify the relationship between Apex and Tuya"
            // rejection. For a bridge, the hub maker is the correct manufacturer
            // (mirrors how SmartThings/Hubitat report bridged devices), and it
            // must stay consistent with the Google company profile. Do NOT
            // restore the per-device passthrough — it re-triggers cert rejection.
